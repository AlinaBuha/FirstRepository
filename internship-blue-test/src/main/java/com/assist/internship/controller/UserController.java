...
import com.assist.internship.config.SecurityConfiguration;
...
public class UserController {
    ...
    public ResponseEntity login(@RequestBody User user) {
        ...
        if (oldUser.getActive() == 1) {
            if (oldUser != null) {
                if(BCrypt.checkpw(user.getPassword(), oldUser.getPassword())) {// if (user.getPassword().equals(oldUser.getPassword()))
...
//************************************************************
    ...
    public ResponseEntity createAdmin(@RequestHeader("reset_token") final String token,@RequestBody User user) {
                ...
                if (!EmailHelper.emailIsValid(email)) {
                    ...
                    if (dbUser != null) {
                       /* Role userRole = roleRepository.findByName("ADMIN");
                        ...
                        user.setPassword(null);
                        return ResponseEntity.status(HttpStatus.OK).body(new InternshipResponse(true, "Admin [" + user.getEmail() + "] created successfully!", Arrays.asList(user)));
                    }
                }
            } else {
                return ResponseEntity.status(HttpStatus.OK).body(new InternshipResponse(false, "Please fill in all the mandatory fields to successfully create the user!", null));
