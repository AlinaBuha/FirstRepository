...
public class CategoryController {
    ...
    public ResponseEntity delCategory(@RequestBody Category category)//, @RequestHeader("reset_token") final String token)
    {
      /*  if (token.isEmpty() || token == null) {
            return ResponseEntity.status(HttpStatus.OK).body(new InternshipResponse(false, "Access Denied", null));
        } else {*/
            //Category dbCategory = categoryService.findByCategoryId(category.getId());
        Category dbCategory = categoryService.findByCategoryName(category.getName());

             if (dbCategory != null) {
               //  Session.delete(dbCategory);
            categoryService.deleteCategory(dbCategory);
            return ResponseEntity.status(HttpStatus.OK).body(new InternshipResponse(true, "Success", null));

        } else {
                return ResponseEntity.status(HttpStatus.OK).body(new InternshipResponse(false, "Not find this category category", null));
